...
- [X] Position-based ICY metadata sync (timeline stores buffer write positions)
- [X] Time-delayed now-playing API (=get-metadata-changed-at= returns epoch ms for client-side sync)
- [X] AAC accumulation buffer flush at track boundaries (prevents audio artifacts at hard transitions)
- [X] Stream quality variants (MP3 128kbps, AAC 128kbps, MP3 64kbps low-bandwidth)
- [X] Robustness: watchdog thread (encoder auto-reinit, audio flow monitoring, track stall detection, playlist thread liveness)

*Future work:*

- [ ] Configurable watchdog thresholds via API
...
Runtime + development headers (needed at build time for CFFI compilation):
...
sudo apt install -y \
  ...
  libtag1-dev libfixposix-dev gcc
#+end_src

- [[https://lame.sourceforge.io/][libmp3lame]] — MP3 encoding (via LAME FFI)
- [[https://github.com/mstorsjo/fdk-aac][libfdk-aac]] — AAC encoding (via C shim, avoids SBCL signal handler conflicts)
- [[https://xiph.org/flac/][libflac]] — FLAC decoding (via cl-mixed-flac)
- [[https://www.mpg123.de/][libmpg123]] — MP3 decoding (via cl-mixed-mpg123)
- [[https://taglib.org/][libtagc]] — audio metadata extraction (taglib C bindings)
- [[https://github.com/sionescu/libfixposix][libfixposix]] — POSIX compatibility (required by iolib for socket networking)
